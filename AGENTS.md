...
## Boundary Facades & Audit Exemptions

- `rust/centrode_core/src/bridge/api.rs` (Rust FFI surface) and `lib/features/graph/store/graph_api.dart` (Dart composite facade) are designated **Boundary Facades**.
- They are deliberately designed as unified, lean gateways holding the cross-boundary interface in one place with trivial cyclomatic complexity (~1 per method, zero business logic, pure delegations).
- They are **exempt from God Object line-count (>500) and public API count (>= 15) thresholds** during code health audits. Do not break them apart unless domain capabilities genuinely diverge.
