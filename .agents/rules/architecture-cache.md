...
- **Compliance Scan**: After making any code changes in a session, run the compliance validation check to ensure you did not introduce any tier leaks or naming violations:
  ...
  dart .agents/plugins/arch-linter/scripts/arch_linter.dart check --rust
  ```
- **Architectural Assertions**: Make sure to check naming conventions and test coverage requirements before completing tasks:
  ...
  dart .agents/plugins/arch-linter/scripts/arch_linter.dart assert_naming --rust
  dart .agents/plugins/arch-linter/scripts/arch_linter.dart assert_tests --dart
  dart .agents/plugins/arch-linter/scripts/arch_linter.dart assert_tests --rust
  ```
