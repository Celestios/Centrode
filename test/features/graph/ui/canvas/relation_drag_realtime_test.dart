...
void main() {
  ...
  testWidgets('RelationLayer correctly rebuilds and propagates dragging overrides', (WidgetTester tester) async {
    ...
    when(() => mockController.nodeLookup).thenReturn({
      'node-from': fromNode,
      'node-to': toNode,
    });
    when(() => mockController.relationLookup).thenReturn({
      'rel-1': rel,
    });
    when(() => mockController.onEntityUpdate).thenAnswer((_) => const Stream.empty());
    ...
    when(() => mockTheme.currentGraphTheme).thenReturn(const GraphTheme(
      id: 'test',
      name: 'test',
    ));
    ...
    final cursorNotifier = ValueNotifier<MouseCursor>(SystemMouseCursors.grab);
    ...
    final customPaint = tester.widgetList<CustomPaint>(customPaintFinder)
  ...
  testWidgets('Gesture dragging relation handle propagates pointer move updates to RelationPainter in real time', (WidgetTester tester) async {
    final mockController = MockGraphDataController();
    final mockTheme = MockThemeController();
    ...
    when(() => mockController.canvasBounds).thenReturn(ValueNotifier(BoundingBox(minX: 0, minY: 0, maxX: 100, maxY: 100)));
