...
void main() {
  group('GraphNodeMutations', () {
    ...
    setUpAll(() {
      ...
      registerFallbackValue(Nodes.iNode(
        INode(
          key: 'dummy',
          fields: INodeFields(
    ...
    test('createNode inserts into store and spatial grid', () async {
      final id = controller.nodeMutations.createNode(UiNodes.info, const Offset(100, 200));
      ...
      final spatialNodes = controller.spatialGrid.queryRect(const Rect.fromLTWH(50, 150, 100, 100));
    ...
    test('deleteNode removes from store immediately optimistically', () async {
      final id = controller.nodeMutations.createNode(UiNodes.task, const Offset(50, 50));
      ...
      await controller.syncEngine.processor.forceFlush();
      verify(() => mockApi.deleteNodeEntry(table: 'TaskNode', key: id)).called(1);
    });

    test('updateNodePosition moves node and updates spatial grid', () {
      final id = controller.nodeMutations.createNode(UiNodes.info, const Offset(0, 0));
