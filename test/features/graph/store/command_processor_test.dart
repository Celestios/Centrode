...
void main() {
  group('CommandProcessor', () {
    ...
    test('overwrites previous pending command of same category', () async {
      ...
      final cmd2 = FakeCommand('node-3', CommandCategory.spatial); // Same ID and Category
    ...
    test('keeps pending commands of different categories', () async {
      ...
      final cmd2 = FakeCommand('node-4', CommandCategory.content); // Different Category
    ...
    test('undoes command on failure and reports error', () async {
      final cmd = FakeCommand('node-5', CommandCategory.lifecycle, shouldFail: true);
