...
import 'package:centrode/features/graph/engine/gesture_interceptor.dart';
...
void main() {
  ...
  group('GestureInterceptor Chain Tests', () {
    ...
    test('Events bubble through multiple interceptors in order', () {
      final interceptor1 = TestInterceptor(
        returnDisposition: InterceptorDisposition.bubble,
      );
    ...
    test(
      'Consumption halts propagation down the chain and bypasses FSM state',
      () {
        final interceptor1 = TestInterceptor(
          returnDisposition: InterceptorDisposition.consumed,
        );
        final interceptor2 = TestInterceptor(
          returnDisposition: InterceptorDisposition.bubble,
        );
  ...
  group('DrawingGestureInterceptor Tests', () {
    ...
    test('captures and consumes pointer events when tool mode is draw', () {
      ...
      expect(
        interceptor.activeStroke.value,
        equals([const Offset(10, 20), const Offset(15, 25)]),
      );
    });
    ...
    test('creates drawing node on pointer up', () {
      ...
      controller.handlePointerMove(
        const PointerMoveEvent(position: Offset(30, 40)),
      );

      // Stub onCreateDrawingNode
      when(
        () => mockEnv.onCreateDrawingNode(
      ...
      controller.handlePointerUp(
        const PointerUpEvent(position: Offset(30, 40)),
      );
