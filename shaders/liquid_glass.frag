...
float unionDistance(vec2 uvCenter, int cnt, float k){
  ...
  for(int i=0;i<MAX_RECTS;++i){
    if(i>=cnt) break;
    vec4 r = getRect(i);
    vec2 hsz  = (r.zw*0.5)/max(R.y, 0.001);
    vec2 posN = (r.xy - 0.5*R)/max(R.y, 0.001);
    float d = sdRoundRect(uvCenter - posN, hsz, getCorner(i)/max(R.y, 0.001));
...
vec3 radialBlur(vec2 uv,float radiusPx){
  ...
  for(int ring=1; ring<=4; ++ring){
    ...
    for(int j=0; j<BLUR_STEPS; ++j){
      ...
      sum += bg(uv + vec2(cos(a) / aspect, sin(a)) * rad);
      cnt++;
    }
  }
...
void main(){
  ...
  for(int i=0;i<MAX_RECTS;++i){
    ...
    vec4 r = getRect(i);
    vec2 hsz  = (r.zw*0.5)/safeRy;
    vec2 posN = (r.xy - 0.5*R)/safeRy;
    d[i]  = sdRoundRect(uvCenter - posN, hsz, getCorner(i)/safeRy);
