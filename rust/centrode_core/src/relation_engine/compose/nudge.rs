use crate::relation_engine::config::NudgingConfig;
...
fn blocked_intervals(
    ...
    let proj_min = group
        .iter()
        .map(|&ri| runs[ri].proj_start)
        .fold(f64::MAX, f64::min);
    let proj_max = group
        .iter()
        .map(|&ri| runs[ri].proj_end)
    ...
    let mut raw: Vec<(f64, f64)> = nodes
        ...
        .filter_map(|node| {
            let (b0, b1) = match orientation {
                ...
                Orientation::Vertical => (node.x - NODE_MARGIN, node.x + node.width + NODE_MARGIN),
                Orientation::Diagonal => unreachable!(),
            };
