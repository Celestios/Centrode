...
use crate::layout_engine::types::{Axis, LayoutPatch};
...
impl GraphService {
    pub async fn set_opt_area(&self, bounds: Option<BoundingBox>) -> anyhow::Result<()> {
        let mut map_data = self.repo.snapshot.get_map_data().await?;
        map_data.opt_area = bounds;
        self.repo.snapshot.update_map_data(map_data).await?;
        Ok(())
    }

    pub async fn get_opt_area(&self) -> anyhow::Result<Option<BoundingBox>> {
        let map_data = self.repo.snapshot.get_map_data().await?;
