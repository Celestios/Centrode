...
use crate::relation_engine::geometry_utils::{
    evaluate_cubic_bezier_point, evaluate_cubic_bezier_tangent,
};
...
impl Shaper for SineWaveShaper {
    fn shape(&self, _raw_path: &[Point], context: &ShaperContext) -> ComputedRelation {
        ...
        let num_samples = ((dist / 4.0).ceil() as usize)
            .max(self.num_samples)
            .max(100);
