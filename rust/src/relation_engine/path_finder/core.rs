...
impl Ord for ANode {
    fn cmp(&self, other: &Self) -> std::cmp::Ordering {
        match other.f.total_cmp(&self.f) {
            std::cmp::Ordering::Equal => match self.col.cmp(&other.col) {
...
pub fn a_star<S: Steer + ?Sized>(steer: &S, context: &AStarContext) -> Option<Vec<Point>> {
    ...
    while let Some(current) = open.pop() {
        ...
        for &(dc, dr, step_cost) in &neighbors {
            ...
            let transition =
                steer.transition_cost(key, nkey, (dc, dr), prev_key, step_cost, context);
